...
export const ObraModal: React.FC<ObraModalProps> = ({ isOpen, onClose, onSaved }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-2xl max-h-[85vh] rounded-[32px] shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 scroll-smooth" style={{ scrollbarWidth: 'thin' }}>
          {isSuccess ? (
            <div className="py-12 text-center animate-in zoom-in-95 duration-300">
              <div className="mx-auto mb-6 w-20 h-20 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center shadow-lg shadow-emerald-100/50">
                <CheckCircle2 size={40} />
              </div>
              <h3 className="text-2xl font-black text-slate-900 mb-3">Obra Salva!</h3>
              ...
              <button
                type="button"
                onClick={onClose}
                className="px-8 py-3 rounded-2xl bg-[#9A077B] text-white font-black shadow-xl shadow-[#EFC6E3] hover:bg-[#7F0665] transition-all hover:scale-105 active:scale-95"
              >
                Entendido
              </button>
            </div>
            ...
            <form className="space-y-5" onSubmit={handleSubmit} id="obraForm">
              {error && (
                <div className="bg-red-50 text-red-600 p-4 rounded-xl flex items-center text-sm font-bold border border-red-100">
